...
const validateDescriptionWordLimit = (description: string): string | null => {
...
const userOwnsConnection = async (userId: string, connectionId: string) => {
  const connectionRes = await queryMetadata(
    'SELECT id FROM user_connections WHERE id = $1 AND user_id = $2',
    [connectionId, userId]
  );
...
export const listNotes = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const notesRes = await queryMetadata(
      `SELECT id, connection_id, name, description, created_at, updated_at
       FROM notes
...
export const createNote = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const countRes = await queryMetadata(
      'SELECT COUNT(*)::int AS count FROM notes WHERE connection_id = $1',
      [connectionId]
    );
    ...
    const insertRes = await queryMetadata(
      `INSERT INTO notes (connection_id, name, description)
       VALUES ($1, $2, $3)
       RETURNING id, connection_id, name, description, created_at, updated_at`,
      [connectionId, payload.name, payload.description]
    );
    ...
    const updateRes = await queryMetadata(
      `UPDATE notes AS n
       ...
       WHERE n.id = $3
         ...
         AND uc.user_id = $4
       RETURNING n.id, n.connection_id, n.name, n.description, n.created_at, n.updated_at`,
      [payload.name, payload.description, noteId, userId]
    );
...
export const deleteNote = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const deleteRes = await queryMetadata(
