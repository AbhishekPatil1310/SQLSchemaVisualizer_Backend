...
import { decrypt } from '../../core/encryption.js'; // Ensure this path is correct
...
// 1. IMPROVED DIALECT DETECTION: Now decrypts first to be 100% sure
const getDialectFromUrl = (encryptedUrl: string): 'postgres' | 'mysql' => {
  try {
...
export const getDatabaseSchema = async (userId: string, encryptedUrl: string) => {
  const pool = await poolManager.getPool(userId, encryptedUrl);
  const client = await pool.connect();
  
  // 2. Detect dialect from the decrypted string
...
export const getVisualSchema = async (userId: string, encryptedUrl: string) => {
