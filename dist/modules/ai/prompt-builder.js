export class PromptBuilder {
    buildQueryGenerationPrompt(userQuery, schemaContext, databaseType) {
...
Do not wrap SQL in markdown blocks.`;
    ...
    buildValidationPrompt(query, databaseType, schemaContext) {
        const schemaDescription = this.formatSchemaForPrompt(schemaContext);
...
Respond as JSON only:
{
  ...
  "indexes": ["CREATE INDEX ..."]
    ...
    buildSchemaSummaryPrompt(schemaContext) {
        const schemaDescription = this.formatSchemaForPrompt(schemaContext);
        ...
        const tableBlocks = schemaContext.tables.map((table) => {
            const columnLines = table.columns.map((column) => {
                ...
                if (column.isForeignKey && column.foreignKeyReference)
                    tags.push(`FK->${column.foreignKeyReference}`);
...
${rowLine}`;
        ...
        const relationshipLines = schemaContext.relationships.length
            ? schemaContext.relationships.map((rel) => `${rel.fromTable}.${rel.fromColumn} -> ${rel.toTable}.${rel.toColumn} (${rel.type})`)
            : ['(none)'];
...
${relationshipLines.join('\n')}`;
    ...
    getDatabaseSpecificHints(databaseType) {
...
- Avoid unnecessary subqueries when joins are clearer.`;
        ...
        const _exhaustive = databaseType;
        return _exhaustive;
    }
}
